...
import com.lgdx.web.MsgMemberMapper;
...
public class MyController {
	...
	public String join(MsgMember mem, Model model) {
		...
		// -> MsgMember생성 -> 요청데이터를 매개변수로 수집
		logger.info("수집한 데이터 확인 >>" + mem);
		// 2. DB에 연결해서 데이터 추가
		mapper.join(mem);
		// model 영역에 회원가잆한 email값만 담아서 다음페이지로 넘겨주기
	...
	public String login(MsgMember mem, HttpSession session) {
		...
		logger.info("login 요청 : email={}, pw=****",mem.getEmail());
		
		// 모든  페이지 기능에서 활용할 수 있는 session Scope 영역을 사용!
		session.setAttribute("member", member);
		
		
		MsgMember loginUser = mapper.login(mem);
...
//		mem.setEmail(mem2.getEmail());
	...
	public String delete(@RequestParam("email") String email) {
		...
		// @RequestParam(): 쿼리 파라미터 값을 답아오기 위해 사용
