...
<body>
	...
	<div id="wrapper">
		...
		<!-- Q6. 로그인을 한 상태에서는 로그인탭 대신 로그아웃탭과 개인정보수정탭을 출력 --> <!--  로그인 전 => 로그인 메뉴 띄우기  -->
		<c:if test="${empty member }">
			<a href="#menu">로그인</a>
		</c:if> <!--  로그인 후 => 개인정보 수정 / 로그아웃메뉴 띄우기  --> <!-- admin 계정의 경우 회원정보관리 메뉴 연결! -->
		...
		<div class="inner">
			...
			<ul class="actions vertical">
				...
				<form action="join" method="post">
					...
					<li><input type="button" value="Email 중복체크"
		...
		<div class="inner">
			...
			<!-- ex) smhrd님 환영합니다.  --> <!-- 로그인 수행 전/후에 따른 h1 태그 출력을 생각해야 한다! -->
			<!-- 로그인 전이라면 => 즉,session 영역에 member가 없다면! --> <!-- empty : null 과 같은 의미 -->
			<c:if test="${empty member }">
				<h1>로그인 한 세션아이디를 출력해주세요</h1>
			</c:if> <!--  로그인 후라면 => 즉 session 영역에 member가 있다면! --> <c:if
				test="${not empty member }">
				<h1>${member.email}님ㅎㅇ.</h1>
			</c:if> </header>
			<div class="content">
				<p>
					게시판을 이용해보세요 ^^<br>
